...
func Validate(
	eng engine.Engine,
	consumer func(engine.LibraryName, engine.RowID, engine.FieldID, error),
) error {
	for _, libraryName := range eng.Librarys() {
		...
		err = libraryValidate(
			eng,
			library,
			func(row engine.RowID, col engine.FieldID, err error) {
...
func libraryValidate(
	eng engine.Engine,
	lib engine.Library,
	consumer func(engine.RowID, engine.FieldID, error),
) error {
